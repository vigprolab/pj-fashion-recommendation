...
def main():
    ...
    if args.mode == 'train':
        param = {'device': device, 'mode': args.mode, 'model_path': args.model_path, 'param_path': args.param_path, 'img_size': args.img_size,
            'dataset_path': args.dataset_path, 'batch_size': args.batch_size, 'num1': args.num_fout1, 'num2': args.num_fout2, 'num3': args.num_fout3, 'lr': args.learning_rate}
        model = models.FRModel(param)
        model.train(args.epochs)

    elif args.mode == 'test':
        param = {'device':device, 'mode':args.mode, 'model_path':args.model_path, 'param_path':args.param_path}
        model = models.FRModel(param)
        model.test(args.test_img_type, args.test_img_path, args.num_recom)
